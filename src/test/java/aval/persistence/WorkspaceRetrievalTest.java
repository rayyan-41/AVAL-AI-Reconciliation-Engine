...
public class WorkspaceRetrievalTest {
    ...
    private void createSchema() throws Exception {
        try (Statement s = conn.createStatement()) {
            s.execute(
                "CREATE TABLE client_organization (org_id UUID PRIMARY KEY, name VARCHAR(100))"
            );
            s.execute(
                "CREATE TABLE reconciliation_workspace (workspace_id UUID PRIMARY KEY, org_id UUID, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP, status VARCHAR(20))"
            );
            s.execute(
                "CREATE TABLE financial_dataset (dataset_id UUID PRIMARY KEY, workspace_id UUID, file_path TEXT, source_type VARCHAR(30), status VARCHAR(20), import_date DATE)"
            );
            s.execute(
                "CREATE TABLE standardized_ledger (transaction_id UUID PRIMARY KEY, value_date DATE, amount DECIMAL(19,4), narrative TEXT, transaction_type VARCHAR(20), source_dataset_id UUID)"
            );
            s.execute(
                "CREATE TABLE standardized_bank (transaction_id UUID PRIMARY KEY, value_date DATE, amount DECIMAL(19,4), narrative TEXT, transaction_type VARCHAR(20), source_dataset_id UUID)"
            );
        }
    }
    ...
    public void testGetLedgerTransactionsForWorkspace_returnsRows()
        ...
        try (Statement s = conn.createStatement()) {
            ...
            s.execute(
                "INSERT INTO financial_dataset (dataset_id, workspace_id, file_path, source_type, status, import_date) VALUES ('" +
                    ...
                    "', '" +
                    wsId +
                    "', '/test.csv', 'INTERNAL_EXCEL', 'PARSED', CURRENT_DATE)"
            );
            s.execute(
                "INSERT INTO standardized_ledger (transaction_id, value_date, amount, narrative, transaction_type, source_dataset_id) VALUES ('" +
                    txId +
                    "', CURRENT_DATE, 1500.00, 'Test Payment', 'DEBIT', '" +
                    dsId +
                    "')"
            );
        }
        ...
        assertFalse(
            ledger.isEmpty(),
            "Should return ledger transactions for workspace"
        );
    ...
    @Test
    public void testGetBankTransactionsForWorkspace_returnsRows()
        throws Exception {
        DataStore ds = new DataStore(new MockDataSource(conn));
        UUID wsId = UUID.randomUUID();
        UUID dsId = UUID.randomUUID();
        ...
        try (Statement s = conn.createStatement()) {
            ...
            s.execute(
                "INSERT INTO financial_dataset (dataset_id, workspace_id, file_path, source_type, status, import_date) VALUES ('" +
                    ...
                    "', '" +
                    wsId +
                    "', '/test.pdf', 'EXTERNAL_PDF', 'PARSED', CURRENT_DATE)"
            );
            s.execute(
                "INSERT INTO standardized_bank (transaction_id, value_date, amount, narrative, transaction_type, source_dataset_id) VALUES ('" +
                    txId +
                    "', CURRENT_DATE, 1500.00, 'Bank Test', 'CREDIT', '" +
                    dsId +
                    "')"
            );
        }

        List<StandardizedTransaction> bank = ds.getBankTransactionsForWorkspace(
            wsId
        );

        assertFalse(
            bank.isEmpty(),
            "Should return bank transactions for workspace"
        );
    ...
    private static class MockDataSource implements javax.sql.DataSource {
        ...
        MockDataSource(Connection conn) {
            this.conn = conn;
        }
